...
bool begin(void) {
  if (HAL_SPI_Init(&hspi2) == HAL_OK) {initialized = true;}
...
void readRegisterN(uint8_t addr, uint8_t buffer[], uint8_t n) {
  ...
  HAL_SPI_Transmit(&hspi2, &addr, 1, SPI_DELAY);
  HAL_SPI_Receive(&hspi2, buffer, n, SPI_DELAY);
...
void writeRegister8(uint8_t addr, uint8_t data) {
  ...
  HAL_GPIO_WritePin(GPIOA, SPI2_CS_Pin, GPIO_PIN_RESET);
  HAL_SPI_Transmit(&hspi2, buffer, 2, SPI_DELAY);  // Corrected size to 2
  HAL_GPIO_WritePin(GPIOA, SPI2_CS_Pin, GPIO_PIN_SET);
}
